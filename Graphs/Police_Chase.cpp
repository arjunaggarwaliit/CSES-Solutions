...
pair<int,vector<bool>> edmonds_karp_max_flow(int source, int sink, const vector<int> adj[], vector<vector<int>>& residual, int n){
    int total_flow = 0;
    while(1){
...
int32_t main(){
    ...
    pair<int,vector<bool>> ans = edmonds_karp_max_flow(1,n,adj,residual,n);
    cout << ans.first << endl;
    vector<bool> visited = ans.second;
    for(int i=1 ; i<=n ; i++){
        if(visited[i] == true){
            for(int j=1 ; j<=n ;j++){
                if(visited[j] == false){
