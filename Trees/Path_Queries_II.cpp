...
struct HLD {
    ...
    void decompose(int node, int h, const vector<vector<int>> &adj) {
        head[node] = h;
        pos[node] = curr++;

        if (heavy[node] != -1) {
            decompose(heavy[node], h, adj);
        }
    ...
    void build(vector<int> &values, const vector<vector<int>>& adj) {
        dfs(1, 0, adj);
        decompose(1, 1, adj);
